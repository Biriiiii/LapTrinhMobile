...
// 🔥 CẬP NHẬT: imageUrl thay vì image
interface Artist {
  id: number;
  name: string;
  imageUrl?: string;
  ...
  const fetchHomeData = async () => {
    try {
      ...
      const [resCat, resArt, resAlb, resProf, resMyAlbums, resFavAlbums] = await Promise.all([
        ...
        apiClient.get('/public/artists'), // 🔥 Lấy danh sách nghệ sĩ
      ...
      // 🔥 CẬP NHẬT QUAN TRỌNG: Xử lý phân trang cho Artist
      if (resArt) {
        // Nếu API trả về dạng Page { content: [...] } thì lấy content
        // Nếu trả về mảng trực tiếp thì lấy data
        const artistList = resArt.data.content || resArt.data;
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          {/* Hiển thị số dư ví nhỏ bên cạnh (Optional) */}
          {profile && (
            <View style={styles.balanceTag}>
              <Text style={styles.balanceText}>{profile.walletBalance.toLocaleString()}đ</Text>
            </View>
          )}
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.scrollPadding}>
          {artists.map((artist) => (
            ...
            >
              <Image
                // 🔥 CẬP NHẬT: Dùng imageUrl cho Artist
                source={{ uri: artist.imageUrl || 'https://via.placeholder.com/100' }}
...
const styles = StyleSheet.create({
  ...
  balanceTag: { backgroundColor: '#333', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 15 },
  balanceText: { color: '#1DB954', fontWeight: 'bold', fontSize: 12 },
  ...
  favIconOverlay: {
    ...
    top: 8,
    right: 8,
    backgroundColor: 'rgba(0,0,0,0.5)', // Làm đậm nền icon tim chút cho dễ nhìn
    padding: 6,
    borderRadius: 20,
  },
  ...
  priceTag: { marginTop: 6, backgroundColor: '#333', alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
